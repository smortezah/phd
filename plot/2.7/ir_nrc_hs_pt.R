...
hs_names_in <- c(
  ...
  "HS17", "HS18", "HS19", "HS20", "HS21", "HS22", "HSX", "HSY", "HSAL", "HSMT", "HSUL", "HSUP"
)
hs_names_out <- c(
...
pt_names_in <- c(
  "PT1", "PT2A", "PT2B", "PT3", "PT4", "PT5", "PT6", "PT7", "PT8", "PT9", "PT10", "PT11", "PT12", "PT13", "PT14",
  "PT15", "PT16", "PT17", "PT18", "PT19", "PT20", "PT21", "PT22", "PTX", "PTY", "PTMT", "PTUL", "PTUP"
)
pt_names_out <- c(
...
df_hs_pt$ref <- factor(df_hs_pt$ref, levels = hs_names_in, labels = hs_names_out)
df_hs_pt$tar <- factor(df_hs_pt$tar, levels = pt_names_in, labels = pt_names_out)

## Data for b
df_diff_hs_pt <- read.table(file_in_b, header = TRUE)
df_diff_hs_pt$ref <- factor(df_diff_hs_pt$ref, levels = hs_names_in, labels = hs_names_out)
df_diff_hs_pt$tar <- factor(df_diff_hs_pt$tar, levels = pt_names_in, labels = pt_names_out)

## Data for c
df_pt_hs <- read.table(file_in_c, header = TRUE)
df_pt_hs$ref <- factor(df_pt_hs$ref, levels = pt_names_in, labels = pt_names_out)
df_pt_hs$tar <- factor(df_pt_hs$tar, levels = hs_names_in, labels = hs_names_out)

## Data for d
df_diff_pt_hs <- read.table(file_in_d, header = TRUE)
df_diff_pt_hs$ref <- factor(df_diff_pt_hs$ref, levels = pt_names_in, labels = pt_names_out)
df_diff_pt_hs$tar <- factor(df_diff_pt_hs$tar, levels = hs_names_in, labels = hs_names_out)
