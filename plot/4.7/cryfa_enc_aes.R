...
df_time <- data.frame(
  method = time_data[, 1], time = as.numeric(time_data[, 2]), format = time_data[, 3], state = time_data[, 4]
)
...
a <- ggplot(df_time, aes(x = method, y = time, fill = state)) +
  ...
  coord_flip() +
  geom_text(
    aes(y = time, label = ifelse(state == "Encrypt", -round(time, 1), round(time, 1))),
    size = 3,
    color = ifelse(df_time$format == "FASTA" & df_time$method == "Cryfa", "black", "white"),
    hjust = ifelse(
...
b <- ggplot(df_size, aes(x = method, y = size, fill = state)) +
  ...
  geom_col() +
  geom_text(aes(y = lab_ypos, label = round(size, digits = 1)), size = 3, color = "white") +
  geom_text(aes(y = total.size, label = paste0("CR = ", round(ratio, digits = 1))), vjust = -0.75, size = 2.5) +
